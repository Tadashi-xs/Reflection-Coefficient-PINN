...
def wavevector_components(psi: float, kappa_value: float) -> tuple[float, float]:
    """Incident wave-vector components in the PDF convention."""
...
def compute_sample(
    ...
    """Compute one physically admissible dataset row."""
...
def generate_rows_serial(
    num_samples: int,
    params: ModelParams,
    seed: int,
    show_progress: bool = True,
) -> tuple[list[dict[str, float]], int]:
    """Generate rows in one process."""
    ...
    iterator = tqdm(total=num_samples, desc="Generating", unit="row", disable=not show_progress)
    ...
    if len(rows) < num_samples:
        raise RuntimeError(
            f"Could generate only {len(rows)} admissible rows out of {num_samples}. "
...
def _worker_generate_chunk(args: tuple[int, ModelParams, int]) -> tuple[list[dict[str, float]], int]:
    chunk_size, params, seed = args
    return generate_rows_serial(chunk_size, params, seed, show_progress=False)


def split_work(total: int, chunk_size: int) -> list[int]:
    if chunk_size <= 0:
...
def generate_dataset(
    ...
    if num_workers <= 1:
        rows, attempts = generate_rows_serial(num_samples, params, seed, show_progress=True)
    else:
        chunks = split_work(num_samples, chunk_size)
        seed_sequence = np.random.SeedSequence(seed)
        child_seeds = [int(s.generate_state(1, dtype=np.uint32)[0]) for s in seed_sequence.spawn(len(chunks))]

        rows = []
        attempts = 0
        worker_args = [(chunk, params, child_seed) for chunk, child_seed in zip(chunks, child_seeds)]

        with ProcessPoolExecutor(max_workers=num_workers) as executor:
            futures = [executor.submit(_worker_generate_chunk, arg) for arg in worker_args]
            with tqdm(total=num_samples, desc=f"Generating ({num_workers} workers)", unit="row") as pbar:
                for future in as_completed(futures):
                    chunk_rows, chunk_attempts = future.result()
                    rows.extend(chunk_rows)
                    attempts += chunk_attempts
                    pbar.update(len(chunk_rows))

        rows.sort(key=lambda r: (r["omega"], r["psi"], r["delta_x"], r["delta_y"]))
...
def print_diagnostics(df: pd.DataFrame, attempts: int) -> None:
    ...
    print(
        ...
        f"q99={df['R'].quantile(0.99):.6e}, "
        f"max={df['R'].max():.6e}"
    )
    print(
        "T: "
        f"min={df['T'].min():.6e}, "
        f"median={df['T'].median():.6e}, "
        f"q99={df['T'].quantile(0.99):.6e}, "
        f"max={df['T'].max():.6e}"
    )
    print(
        "physics residuals: "
        f"max_res1={df['residual_1'].max():.6e}, "
        f"max_res2={df['residual_2'].max():.6e}"
    )
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Generate a clean synthetic dataset for Forward PINN.")

    parser.add_argument("--num_samples", type=positive_int, default=30_000)
    parser.add_argument("--output", type=str, default="storage/dataset_30k_clean.csv")
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--num_workers", type=positive_int, default=1)
...
def main() -> None:
    ...
    if args.num_workers > os.cpu_count():
        print(f"Warning: num_workers={args.num_workers} is greater than os.cpu_count()={os.cpu_count()}.")
    ...
    lattice = LatticeParams(a=args.period, n_terms=args.n_terms)
